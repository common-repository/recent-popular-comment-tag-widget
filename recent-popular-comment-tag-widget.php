<?php
/*
 * Plugin Name: Recent Popular Comment Tag Widget
 * Plugin URI: https://abunaserkhan.tk/recent-popular-comment-tag-widget/
 * Description: Enables advanced widget that gives you total control over the output of your site’s recent posts, popular posts, comments, tags.
 * Author: Md. Abunaser Khan
 * Author URI: https://abunaserkhan.tk
 * Version: 1.0
 * License: GPL2
**/
...
/*
    ...
    Copyright (C) 2018  Md. Abunaser Khan  (mdabunaserkhanbd@gmail.com) All rights reserved
...
include_once('inc/rpctw_post_script.php');


class recent_popular_comment_tag_widget extends WP_Widget {

/*  Constructor
/* ------------------------------------ */
	function __construct() {
		parent::__construct('rpctwtabs', __('Recent Popular Comment Tag', 'recent-popular-comment-tag-widget'),
		array(
			'description' => __('List posts, comments, and/or tags with or without tabs.', 'recent-popular-comment-tag-widget'),
			'classname' => 'widget_rpctw_tabs'
		)
		);
	}


  public function rpctw_get_defaults() {
    return array(
      'title'       => '',
      'tabs_category'   => 1,
      'tabs_date'     => 1,
...
/* ------------------------------------ */
	private function _create_tabs($tabs,$count) {
		...
		$titles = array(
			'recent'	=> __('Recent Posts','recent-popular-comment-tag-widget'),
			'popular'	=> __('Popular Posts','recent-popular-comment-tag-widget'),
			'comments'	=> __('Recent Comments','recent-popular-comment-tag-widget'),
			'tags'		=> __('Tags','recent-popular-comment-tag-widget')
		);
		$icons = array(
		...
		$output = sprintf('<ul class="rpctw-tabs-nav group tab-count-%s">', $count);
		foreach ( $tabs as $tab ) {
			$output .= sprintf('<li class="rpctw-tab tab-%1$s"><a href="#tab-%2$s" title="%4$s"><i class="%3$s"></i><span>%4$s</span></a></li>',
        $tab,
        $tab . '-' . $this -> number,
        $icons[$tab],
        $titles[$tab]
...
/* ------------------------------------ */
	public function widget($args, $instance) {
		extract( $args );
    $defaults = $this -> rpctw_get_defaults();
...
?>
	...
	<div class="rpctw-tabs-container">
		...
		<?php if($instance['recent_enable']) { // Recent posts enabled? ?>
			...
			<ul id="tab-recent-<?php echo $this -> number ?>" class="rpctw-tab group <?php if($instance['recent_thumbs']) { echo 'thumbs-enabled'; } ?>">
				...
				<li>
					...
					<div class="tab-item-thumbnail">
						<a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>">
							...
							<?php else: ?>
							  <img src="<?php echo plugins_url( 'assets/images/thumb-medium-empty.png', __FILE__ ); ?>" />
							<?php endif; ?>
		...
		<?php if($instance['popular_enable']) { // Popular posts enabled? ?>
			...
			?>
			<ul id="tab-popular-<?php echo $this -> number ?>" class="rpctw-tab group <?php if($instance['popular_thumbs']) { echo 'thumbs-enabled'; } ?>">
		...
		<?php if($instance['comments_enable']) { // Recent comments enabled? ?>
			...
			<ul id="tab-comments-<?php echo $this -> number ?>" class="rpctw-tab group <?php if($instance['comments_avatars']) { echo 'avatars-enabled'; } ?>">
				...
				<li>
						...
						<div class="tab-item-inner group">
							...
							<div class="tab-item-name"><?php echo esc_attr( $comment->comment_author ); ?> <?php _e('says:','recent-popular-comment-tag-widget'); ?></div>
		...
		<?php if($instance['tags_enable']) { // Tags enabled? ?>
			<ul id="tab-tags-<?php echo $this -> number ?>" class="rpctw-tab group rpctw-tags">
				<li>
					<?php wp_tag_cloud(); ?>
				</li>
			</ul><!--/.rpctw-tab-->
...
?>
	...
	<style>
	.widget .widget-inside .rpctw-options-tabs .postform { width: 100%; }
	.widget .widget-inside .rpctw-options-tabs p { margin: 3px 0; }
	.widget .widget-inside .rpctw-options-tabs hr { margin: 20px 0 10px; }
	.widget .widget-inside .rpctw-options-tabs h4 { margin-bottom: 10px; }
	</style>

	<div class="rpctw-options-tabs">
		<p>
...
/*  Register widget
/* ------------------------------------ */
if ( ! function_exists( 'recent_popular_comment_tag_widget_tabs' ) ) {

	function recent_popular_comment_tag_widget_tabs() {
		register_widget( 'recent_popular_comment_tag_widget' );
	}

}
add_action( 'widgets_init', 'recent_popular_comment_tag_widget_tabs' );
